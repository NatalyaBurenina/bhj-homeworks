...
function addTask(event) {
	...
	const task = `<div class="task"><div class="task__title">${text.value}</div><a href="#" class="task__remove">&times;</a></div>`;
...
function init() {
	if (!localStorage.tasks) {
		...
		for (let title of JSON.parse(localStorage.tasks)) {
			const task = `<div class="task"><div class="task__title">${title}</div><a href="#" class="task__remove">&times;</a></div>`;
			taskList.insertAdjacentHTML('beforeEnd', task);
		}
		canBeDeleted();
	}
